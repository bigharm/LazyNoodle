...
**LazyNoodle** 是一个AI驱动的自由叙事游戏引擎。由玩家提供一个世界观（txt文件），世界观可以只有几十字，又或者几千上万字。玩家完全可以编写自己的世界观，或者用AI从现有的小说中提取（比如《哈利波特》《射雕英雄传》...）
AI会根据世界观生成相应的地点和少量初始NPC，玩家可以跟NPC互动对话，探索世界。一切内容都是基于世界观规则产生。
V1版本的地点会随着跟NPC互动动态新增。但是NPC仍然需要玩家手动添加。添加简单的NPC只需要几十字的描述，更加丰满复杂的NPC则需要玩家提供更多的描述。

目前只使用Deepseek调试过，使用别的AI 生成的输出不一定适配，影响游戏体验。
